...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 8:
        pairs = pairs[modulo_page * 8:8 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("⬅️ ᴘяєνισυѕ🔹", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("✧ вαcк ✧", callback_data="yone_back"),
             EqInlineKeyboardButton("🔹ɴєχт ➡️", callback_data="{}_next({})".format(prefix, modulo_page)))]

    else:
        pairs += [[EqInlineKeyboardButton("✧ вαcк ✧", callback_data="yone_back")]]
